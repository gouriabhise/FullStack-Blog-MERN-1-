...
import { useFetchRelatedBlgosQuery } from '../../../redux/features/Blogs/blogsApi'

const RelatedBlogs = () => {
    const {id}=useParams
    const {data:blogs=[],error,isLoading}=useFetchRelatedBlgosQuery()
  return (
    <div>
        ...
        blogs.length>0?(<div className='space-y-4 mt-5'>
{
    blogs.map((blog)=>(
        ...
        <div>
            <h4 className='font-medium text-blue-50'>{blog?.title.substring(0,50)}</h4>
